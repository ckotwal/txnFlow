...
public class TransactionFlowResourceIntTest {
    ...
    public void setup() {
        ...
        final TransactionFlowResource transactionFlowResource = new TransactionFlowResource(transactionFlowRepository);
    ...
    public void updateTransactionFlow() throws Exception {
        ...
        TransactionFlow updatedTransactionFlow = transactionFlowRepository.findById(transactionFlow.getId()).get();
        // Disconnect from session so that the updates on updatedTransactionFlow are not directly saved in db
        em.detach(updatedTransactionFlow);
    ...
    public void equalsVerifier() throws Exception {
        ...
        TransactionFlow transactionFlow1 = new TransactionFlow();
        transactionFlow1.setId(1L);
        TransactionFlow transactionFlow2 = new TransactionFlow();
        transactionFlow2.setId(transactionFlow1.getId());
        assertThat(transactionFlow1).isEqualTo(transactionFlow2);
        transactionFlow2.setId(2L);
        assertThat(transactionFlow1).isNotEqualTo(transactionFlow2);
        transactionFlow1.setId(null);
        assertThat(transactionFlow1).isNotEqualTo(transactionFlow2);
    }
}
